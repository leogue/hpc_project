...
\newcommand{\coverpage}{
    \begin{titlepage}
        ...
        \vspace{0.7cm}
        {\large \url{https://github.com/leogue/hpc_project}\par}
        \vfill
        {\large\textbf{Authors:}}\par
        {\large CHABAGNO Antonin\par
        Léo GUERIN\par}
        \vspace{2cm}
...
\section{Exercise 1: Series Summation}
...
\begin{figure}[H]
    ...
    \caption{Execution time vs number of threads / procs}
    \label{fig:omp_mpi}
\end{figure}
...
\begin{enumerate}
\item Communication Overhead: The operations $ MPI\_Scatterv $ and $ MPI\_Gatherv $ require Rank 0 to serialize and transmit the entire dataset (approx 1.2 GB for $N=10^8$) to other processes. In a shared-memory context, this involves expensive memory-to-memory copies that are slower than direct pointer access used in OpenMP.
