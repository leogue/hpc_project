...
void Read_vector(char prompt[], double local_vec[], int n, int local_n,
                 int my_rank, MPI_Comm comm);
void Generate_matrix(double local_A[], int m, int local_m, int n, int my_rank,
                     MPI_Comm comm);
void Generate_vector(double local_vec[], int n, int local_n, int my_rank,
                     MPI_Comm comm);
...
int main(void) {
  ...
  // To use stdin input, uncomment Get_dims and Read_ calls
  // Get_dims(&m, &local_m, &n, &local_n, my_rank, comm_sz, comm);

  // Hardcoded size for adaptation to random vector product
  m = 10000;
  n = 10000;
  local_m = m / comm_sz;
  local_n = n / comm_sz;
  ...
  if (my_rank == 0) {
    printf("Dense MPI Matrix-Vector Multiplication\n");
...
void Allocate_arrays(double **local_A_pp /* out */,
                     ...
                     double **local_y_pp /* out */, int local_m /* in  */,
                     int n /* in  */, int local_n /* in  */,
                     MPI_Comm comm /* in  */) {
 ...
 * Purpose:   Generate the tridiagonal matrix (stored densely) and distribute
 */
void Generate_matrix(double local_A[] /* out */, int m /* in  */,
 ...
 * Purpose:   Generate random vector and distribute
 */
void Generate_vector(double local_vec[] /* out */, int n /* in  */,
...
void Print_matrix(char title[] /* in */, double local_A[] /* in */,
  ...
  if (my_rank == 0) {
    ...
    Check_for_error(local_ok, "Print_matrix", "Can't allocate temporary matrix",
                    comm);
    MPI_Gather(local_A, local_m * n, MPI_DOUBLE, A, local_m * n, MPI_DOUBLE, 0,
               comm);
    printf("\nThe matrix %s\n", title);
    for (i = 0; i < m; i++) {
      for (j = 0; j < n; j++)
...
void Mat_vect_mult(double local_A[] /* in  */, double local_x[] /* in  */,
                   double local_y[] /* out */, int local_m /* in  */,
                   int n /* in  */, int local_n /* in  */,
                   MPI_Comm comm /* in  */) {
  double *x;
  int local_i, j;
  ...
  if (x == NULL)
    local_ok = 0;
  Check_for_error(local_ok, "Mat_vect_mult", "Can't allocate temporary vector",
                  comm);
